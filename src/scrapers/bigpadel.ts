...
export class BigPadelScraper implements BookingProvider {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                for (const court of data) {
                    if (!court.activities) continue;

                    for (const activity of court.activities) {
                        if (activity.id !== PADEL_ACTIVITY_ID) continue;
                        ...
                        if (activity.slots) {
                            for (const slot of activity.slots) {
                                ...
                                if (slot.prices && slot.prices.length > 0) {
                                    ...
                                    isBookable = slot.prices.some((p: any) => p.bookable === true);
                                ...
                                let price = 0;
                                if (slot.prices && slot.prices.length > 0) {
                                    const validPrice = slot.prices.find((p: any) => p.bookable === true) || slot.prices[0];
