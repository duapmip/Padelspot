...
export class Padel33Scraper {
    ...
    async ensureLoggedIn() {
        ...
        try {
            ...
            const initCookieStr = setCookie.map((c) => c.split(';')[0]).join('; ');
            // Extract tokens
            ...
            if (viewStateMatch)
                params.append('__VIEWSTATE', viewStateMatch[1]);
            if (viewStateGenMatch)
                params.append('__VIEWSTATEGENERATOR', viewStateGenMatch[1]);
            if (eventValMatch)
                params.append('__EVENTVALIDATION', eventValMatch[1]);
            ...
            const allCookiesStr = [...setCookie, ...loginCookies].map((c) => c.split(';')[0]).join('; ');
